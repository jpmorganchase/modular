...
function formatMessage(message: string | { message: string }) {
  ...
  let formatted_message = lines.join('\n');
...
export default function formatWebpackMessages(json: {
  ...
  const formattedErrors = json.errors ? json.errors.map(formatMessage) : [];
  const formattedWarnings = json.warnings
    ? json.warnings.map(formatMessage)
    : [];
