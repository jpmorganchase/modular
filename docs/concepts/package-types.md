...
entrypoint is `src/index.tsx` in their manifest and they are built and tested
exactly like `package` types. The only difference is that, since `view`s should
always export a React component, they can be [`start`ed](../commands/start.md)
to spawn a local developer server and render their default export to the DOM.
