...
| `esm-view` | Yes             | Yes             | Yes            | Yes            | No                         | Yes + rewriting external dependencies to CDN | The compiled bundle will use `src/index.tsx` as an entrypoint and [link it](../esm-views/output-package-manifest.md) in the output `package.json`; the entrypoint needs to export a default (non-named) React componentt for `modular start`/ [standalone `index.html`](../esm-views/how-to-build.md) to work |
